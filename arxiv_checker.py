...
logging.basicConfig(
    filename=os.path.join(sys.path[0], "info.log"),
    level=logging.INFO,
...
if now.weekday() != 0 and now - last_digest < timedelta(weeks=1):
    logger.info(
        "Abort: it's not Monday, and it's been less than a week since last digest"
    )
    sys.exit()
    ...
    for section in config.sections():
        ...
        for author in config[section].keys():
            ...
            results[section][author] = [
                paper
                for paper in arxiv.query(
                    query=build_author_query(author),
                    max_results=5,
                    sort_by="lastUpdatedDate",
                    sort_order="descending",
                )
                if datetime.fromtimestamp(timegm(paper["updated_parsed"]))
                >= last_digest
            ]
